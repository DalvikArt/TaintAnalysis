...
// check if the register has been tainted
bool checkAlreadyRegTainted(REG reg)
{
     for(list<REG>::iterator i = regsTainted.begin(); i != regsTainted.end(); i++)
     {
          if (*i == reg)
               return true;
     }
...
// add the register and its sub register into tainted list
...
// remove the register and its sub register from tainted list
...
VOID RegisterHandler(INS ins)
{
     ...
     if (REG_valid(reg_w))
     {
          // remove the write register from list if the read register has not been tainted
          ...
          // add the write regisster into the tainted list if the read register has been tainted
